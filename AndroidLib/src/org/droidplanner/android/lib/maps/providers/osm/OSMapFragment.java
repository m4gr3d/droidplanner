package org.droidplanner.android.lib.maps.providers.osm;
 ...
 */
public class OSMapFragment extends Fragment implements BaseDPMap {
	...
	private final Marker.OnMarkerDragListener mMarkerDragHandler = new Marker.OnMarkerDragListener() {
		...
		public void onMarkerDrag(Marker marker) {
			if (mMarkerDragListener != null) {
				...
				markerInfo.setPosition(BaseMapUtils.GeoPointToCoord(marker.getPosition()));
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		mAppPrefs = new BaseDroidPlannerPrefs(activity.getApplicationContext());
				...
				new MapEventsReceiver() {
					...
					public boolean singleTapConfirmedHelper(GeoPoint iGeoPoint) {
						if (mMapClickListener != null) {
							mMapClickListener.onMapClick(BaseMapUtils.GeoPointToCoord(iGeoPoint));
							return true;
						}
						return false;
					}
					...
					public boolean longPressHelper(GeoPoint iGeoPoint) {
						if (mMapLongClickListener != null) {
							mMapLongClickListener.onMapLongClick(BaseMapUtils
	...
	public void goToMyLocation() {
		...
		if (myLocation != null) {
			updateCamera(BaseMapUtils.GeoPointToCoord(myLocation), currentZoomLevel);
		}
	}
	...
	public void addFlightPathPoint(Coord2D coord) {
		final GeoPoint position = BaseMapUtils.CoordToGeoPoint(coord);
	...
	public List<Coord2D> projectPathIntoMap(List<Coord2D> path) {
		...
		for (Coord2D point : path) {
			...
			coords.add(BaseMapUtils.GeoPointToCoord(coord));
		}
	...
	public void updateMarker(BaseMarkerInfo markerInfo, boolean isDraggable) {
		final GeoPoint position = BaseMapUtils.CoordToGeoPoint(markerInfo.getPosition());
	...
	public void updateDroneLeashPath(PathSource pathSource) {
		...
		for (Coord2D coord : pathCoords) {
			geoPoints.add(BaseMapUtils.CoordToGeoPoint(coord));
		}
		...
		if (mDroneLeashPath == null) {
			...
			mDroneLeashPath.setWidth(BaseMapUtils.scaleDpToPixels(DRONE_LEASH_DEFAULT_WIDTH,
                    getResources()));
